...
def main():
    print("Carregando dataset por nome...")
    ...
    codebook = treinar_codebook(imagens_documentos, n_clusters=50)

    print("Indexando documentos...")
    indice = indexar_documentos(imagens_documentos, codebook)
    ...
    for nome_query in INDICES_QUERIES:
        ...
        salvar_resultados(
            query_img,
            ranking_score,
            nome_query=f"bovw_score_query_{nome_query}",
        ...
        salvar_resultados(
            query_img,
            ranking_iou,
            nome_query=f"bovw_iou_query_{nome_query}",
            indice_query=nome_query,
            label_query=nome_query,
            bbox_query=bbox_query,
